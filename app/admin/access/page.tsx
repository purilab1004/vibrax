...
function Bars({ items, color = '#2563eb', fmt = (k: string) => k }: { items: KV[]; color?: string; fmt?: (k: string) => string }) {
  ...
  return <ul className="p-4 space-y-2">{items.map(i => <li key={i.k}><div className="flex items-center justify-between text-[13px]"><span className="truncate text-[#241f17]">{fmt(i.k)}</span><span className="tabular-nums text-[#6b6152] ml-3">{i.v.toLocaleString()}</span></div><div className="h-1 rounded-full bg-[#f1ece2] mt-1 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${(i.v / max) * 100}%`, background: color }} /></div></li>)}{items.length === 0 && <li className="text-[12px] text-[#9d9280]">데이터 없음</li>}</ul>
}
...
export default function AdminAccessPage() {
  ...
  if (state.days === days && state.err) return <div>{header}<Card className="p-6 text-[13px] text-[#857a68]">{state.missing ? <>접속 로그 테이블이 없어요. <code>db/migrations/2026-08-18-logs.sql</code> 을 실행하세요.</> : state.err}</Card></div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card className="overflow-hidden">
          ...
          {data.recentUsers.length === 0 ? <p className="p-5 text-[13px] text-[#9d9280]">데이터 없음</p> : (
            <ul className="divide-y divide-[#f0eadf] max-h-[460px] overflow-y-auto">{data.recentUsers.map(u => (
              <li key={u.id} className="flex items-center gap-3 px-5 py-2.5 text-[13px]">
                <Avatar url={u.avatar} name={u.name} size={30} />
                <div className="flex-1 min-w-0"><p className="font-semibold text-[#241f17] truncate">{u.name} {u.role === 'admin' && <Badge color="#e11d48">ADMIN</Badge>}</p><p className="text-[11px] text-[#9d9280] truncate">{u.path} · {flagOf(u.country)} {u.device} · {u.browser}</p></div>
                <span className={`text-[11px] whitespace-nowrap ${now - new Date(u.at).getTime() < 5 * 60_000 ? 'text-emerald-600 font-semibold' : 'text-[#9d9280]'}`}>{now - new Date(u.at).getTime() < 5 * 60_000 ? '● 접속 중' : rel(u.at)}</span>
              </li>))}</ul>
          )}
        </Card>
        <Card className="overflow-hidden">
