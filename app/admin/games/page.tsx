...
export default function AdminGamesPage() {
  ...
  return (
    <div>
      <h1 className="font-pixel text-[#0284c7] text-base tracking-widest mb-6">{a.gamesHeading}</h1>
      <div className="flex gap-3 mb-6 flex-wrap">
        <form onSubmit={e => { e.preventDefault(); load() }} className="flex-1 min-w-[200px] max-w-sm">
          <input
            ...
            className="w-full bg-[#ffffff] border border-[#ddd3bf] focus:border-[#0284c7] px-4 py-2.5 text-base outline-none text-[#241f17] placeholder-[#a1957f]"
          />
        </form>
          ...
          {(['newest', 'views'] as const).map(s => (
            <button
              ...
              className={`font-pixel text-[11px] tracking-widest px-3 py-2 border transition-colors ${
                sort === s ? 'border-[#0284c7] text-[#0284c7]' : 'border-[#ebe4d6] text-[#857a68] hover:text-[#241f17]'
              }`}
            >
                ...
                <tr key={g.id}>
                  <td className="px-4 py-3">
                    {editingId === g.id ? (
                      <input
                        ...
                        className="w-full bg-[#ffffff] border border-[#0284c7] px-2 py-1 text-sm outline-none text-[#241f17]"
                      />
                    ) : (
                      <a href={g.play_url} target="_blank" rel="noreferrer" className="text-[#241f17] hover:text-[#0284c7] transition-colors">
                        {g.title}
                      </a>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    {editingId === g.id ? (
                      <select
                        ...
                        className="bg-[#ffffff] border border-[#0284c7] px-2 py-1 text-sm outline-none text-[#241f17]"
                      >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      {editingId === g.id ? (
                        <>
                          <button onClick={saveEdit} className="font-pixel text-[10px] bg-[#0284c7] text-[#241f17] px-2 py-1">{a.save}</button>
                        ...
                        <>
                          <button onClick={() => startEdit(g)} className="font-pixel text-[10px] border border-[#ddd3bf] text-[#6b6152] px-2 py-1 hover:border-[#0284c7] hover:text-[#0284c7] transition-colors">{a.edit}</button>
