...
export default function AdminGamesPage() {
  ...
  const [form, setForm] = useState({ title: '', genre: 'action' as Genre, coin_cost: 1, teaser: '' })
  ...
  const openEdit = (g: GameWithCreator) => { setEditing(g); setForm({ title: g.title, genre: g.genre, coin_cost: g.coin_cost ?? 1, teaser: g.teaser ?? '' }) }
  const saveEdit = async () => {
    if (!editing) return
    setBusy(true)
    const { error } = await supabase.from('games').update({ title: form.title.trim(), genre: form.genre, coin_cost: Math.max(0, form.coin_cost | 0), teaser: form.teaser.trim() || null } as never).eq('id', editing.id)
    setBusy(false)
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {games === null ? <Skeleton /> : list.length === 0 ? <EmptyState icon="🎮" title="게임이 없어요" desc="검색어나 필터를 바꿔 보세요." /> : (
              ...
              <tbody className="divide-y divide-[#f0eadf]">
                {list.map(g => (
                  <tr key={g.id} className={trHover}>
                    ...
                    <td className={td}><span className="text-[#4a4337]">{g.profiles?.agent_name ?? g.profiles?.username ?? '-'}</span></td>
