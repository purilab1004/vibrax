        ...
        <Card className={`p-5 border-l-4 ${st.blocked ? 'border-l-[#dc2626]' : danger ? 'border-l-[#dc2626]' : warn ? 'border-l-[#f59e0b]' : 'border-l-[#059669]'}`}>
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex flex-col gap-2 min-w-[260px]">
              <div className="flex items-center justify-between gap-3"><span className="text-[12px] font-semibold text-[#6b7280]">모드</span><Segmented value={g.mode} onChange={v => setGuard({ ...guard, guard: { ...g, mode: v } })} options={[{ value: 'auto', label: '오토' }, { value: 'manual', label: '매뉴얼' }, { value: 'off', label: '끔' }]} /></div>
              {g.mode === 'auto' && <>
                <div className="flex items-center justify-between gap-3"><span className="text-[12px] font-semibold text-[#6b7280]">한도 (원가/매출)</span><div className="flex items-center gap-1"><input type="number" min={10} max={200} step={5} value={Math.round(g.maxRatio * 100)} onChange={e => setGuard({ ...guard, guard: { ...g, maxRatio: Number(e.target.value) / 100 } })} className={inputCls + ' !w-20 text-right'} /><span className="text-[12px] text-[#6b7280]">%</span></div></div>
                <div className="flex items-center justify-between gap-3"><span className="text-[12px] font-semibold text-[#6b7280]">기간</span><Segmented value={g.window} onChange={v => setGuard({ ...guard, guard: { ...g, window: v } })} options={[{ value: 'day', label: '일' }, { value: 'month', label: '월' }]} /></div>
                <div className="flex items-center justify-between gap-3"><span className="text-[12px] font-semibold text-[#6b7280]" title="매출이 이 금액 미만이면 비율 대신 절대 원가(최소매출×한도)로 판단">최소 매출 기준 ($)</span><input type="number" min={0} step={5} value={g.minRevenueUsd} onChange={e => setGuard({ ...guard, guard: { ...g, minRevenueUsd: Number(e.target.value) } })} className={inputCls + ' !w-20 text-right'} /></div>
              </>}
              <button onClick={() => patchGuard({ mode: g.mode, maxRatio: g.maxRatio, window: g.window, minRevenueUsd: g.minRevenueUsd })} className={btn.primary + ' justify-center mt-1'}>가드 저장</button>
