...
export default function AdminLogsPage() {
  ...
  if (state.days === days && state.err) return <div>{header}<Card className="p-6 text-[13px] text-[#857a68]">{state.missing ? <>로그 테이블이 없어요. <code>db/migrations/2026-08-18-logs.sql</code> 을 실행하세요.</> : state.err}</Card></div>
              ...
              {groups.map(x => (
                <>
                  <tr key={x.fingerprint} className={`${trHover} cursor-pointer ${x.resolved ? 'opacity-50' : ''}`} onClick={() => setOpenFp(openFp === x.fingerprint ? null : x.fingerprint)}>
                    <td className={td}><p className="font-semibold text-[#241f17] max-w-[520px] truncate font-mono text-[12.5px]">{x.message}</p><p className="text-[11px] text-[#9d9280] truncate max-w-[520px]">{x.path ?? '-'}{x.level === 'warn' ? ' · warn' : ''}</p></td>
                    ...
                    <td className={`${td} whitespace-nowrap text-[#857a68]`}>{new Date(x.last).toLocaleString()}</td>
                    <td className={td} onClick={e => e.stopPropagation()}><div className="flex gap-1.5 justify-end">
                      ...
                      <button onClick={() => setConfirm({ fp: x.fingerprint })} className="inline-flex items-center h-8 px-2.5 rounded-lg border border-[#ebe4d6] text-[12.5px] text-[#857a68] hover:border-[#e11d48] hover:text-[#e11d48]">삭제</button>
                    </div></td>
                  </tr>
                  {openFp === x.fingerprint && (
                    <tr key={x.fingerprint + '-d'}><td colSpan={7} className="px-5 py-4 bg-[#faf8f3]">
                      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4 text-[12.5px]">
                        ...
                        <div className="space-y-1.5 text-[#4a4337]">
                          <p><span className="text-[#9d9280]">처음</span> {new Date(x.first).toLocaleString()}</p>
                          <p><span className="text-[#9d9280]">마지막</span> {new Date(x.last).toLocaleString()}</p>
                          <p><span className="text-[#9d9280]">사용자</span> {x.sample.user_id ?? '비로그인'}</p>
                          <p className="break-all"><span className="text-[#9d9280]">UA</span> {x.sample.user_agent ?? '-'}</p>
                          {x.sample.meta ? <pre className="text-[11px] bg-white border border-[#ebe4d6] rounded-lg p-2 overflow-x-auto">{JSON.stringify(x.sample.meta, null, 1)}</pre> : null}
                        </div>
                      </div>
                    </td></tr>
                  )}
