...
export default async function AdminAdsPage() {
  ...
  return (
    <div>
      <PageHeader title="AJ AdPilot" desc="AJ가 운영하는 홍보 캠페인(의뢰) 현황. 클릭당 코인이 소진되고, 소진 코인이 플랫폼 광고 수익입니다." actions={<Link href="/ads" className="inline-flex items-center h-9 px-3.5 rounded-lg border border-[#ddd3bf] bg-white text-[13px] font-medium text-[#4a4337] hover:border-[#2563eb] hover:text-[#2563eb]">캠페인 만들기 화면</Link>} />
          ...
          <div className="overflow-x-auto"><table className="w-full">
            ...
            <tbody className="divide-y divide-[#f0eadf]">{rows.map(r => (
              <tr key={r.id} className={trHover}>
                ...
                <td className={`${td} text-right tabular-nums`}>{r.spent_coins}/{r.budget_coins} <span className="text-[#9d9280]">@{r.cpc_coins}</span></td>
                ...
                <td className={`${td} whitespace-nowrap text-[#857a68]`}>{new Date(r.created_at).toLocaleDateString()}</td>
              </tr>))}</tbody>
          </table></div>
        )}
      </Card>
