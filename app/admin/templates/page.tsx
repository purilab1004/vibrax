...
interface Data { static: { slug: string; name: string; keywords: string[]; prompt: string }[]; db: DbT[] }
...
export default function AdminTemplatesPage() {
  ...
  if (err) return <div>{header}<Card className="p-6 text-[13px] text-[#6b7280]">{err.missing ? <>테이블이 없어요. <code>db/migrations/2026-08-19-studio-templates.sql</code> 을 실행하세요.</> : err.msg}</Card></div>
  ...
  return (
    <div>
      ...
      <Card className="overflow-hidden mb-3">
        <div className="px-4 py-2.5 border-b border-[#e3e6ec] text-[12px] font-bold uppercase tracking-wide text-[#1f2430]">정적 템플릿</div>
        <div className="flex flex-wrap gap-2 p-3">{data.static.map(t => <span key={t.slug} className="inline-flex items-center gap-2 rounded-md border border-[#e3e6ec] bg-[#f7f8fa] px-2.5 h-8 text-[12.5px]"><b>{t.name}</b><span className="text-[#9aa1ad]">{t.keywords.slice(0, 3).join(' · ')}</span><button onClick={() => setPreview({ id: t.slug, name: t.name })} className="text-[#2563eb] hover:underline">미리보기</button></span>)}</div>
      </Card>
