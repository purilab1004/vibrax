...
export default function BlogPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      <h1 className="font-pixel text-[#2563eb] text-base tracking-widest mb-6">{b.heading}</h1>
      ...
      <div className="flex gap-8 items-start">
              ...
              >
                {p.thumbnail_url ? (
                  ...
                  <div className="hidden sm:flex w-32 h-24 rounded-lg border border-[#ebe4d6] shrink-0 items-center justify-center bg-[#ffffff]">
                    <span className="font-pixel text-[#2563eb]/25 text-[10px]">VIBREX<span className="text-[#c9940c]/25">CUP</span></span>
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-[#857a68] mb-1">
                    {catName(p.category_id) && <span className="text-[#2563eb] font-medium mr-2">{catName(p.category_id)}</span>}
                    {p.published_at ? new Date(p.published_at).toLocaleDateString('ko-KR') : ''}
                  </p>
                  <h2 className="text-[17px] font-semibold text-[#241f17] leading-snug line-clamp-2 group-hover:text-[#2563eb] transition-colors">
                    {p.title}
                  </h2>
        ...
        <aside className="hidden lg:block w-80 shrink-0 sticky top-20">
          <div className="border border-[#ebe4d6] bg-[#ffffff] rounded-xl overflow-hidden">
            <p className="px-5 py-3.5 border-b border-[#ebe4d6] font-pixel text-[11px] text-[#2563eb] tracking-widest">
              🔥 {b.popular}
            </p>
              ...
              {popular.map((p, i) => (
                <Link key={p.id} href={`/blog/${p.id}`} className="flex gap-3 px-5 py-3.5 hover:bg-[#241f17]/5 transition-colors group">
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm text-[#3a332a] leading-snug line-clamp-2 group-hover:text-[#2563eb] transition-colors">{p.title}</span>
