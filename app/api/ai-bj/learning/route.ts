...
export async function GET() {
  ...
  const { data, error } = await admin.from('aj_play_policies')
    .select('game_id,version,rules,tips,best_score,best_score_at,auto_learn,auto_count,template_skill,last_skill_at,episodes,demos,updated_at,games(title,genre,thumbnail_url,studio_project_id)')
