...
const PADDLE_ENV = process.env.NEXT_PUBLIC_PADDLE_ENV  // 'sandbox' | 'production' — 미설정이면 결제 비활성 (잘못된 계정으로 붙지 않게)
const paddleConfigured =
  (PADDLE_ENV === 'sandbox' || PADDLE_ENV === 'production') && !!process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN && CREDIT_PACKS.every(p => packPriceId(p.key))

export default function CreditsClient({ countryCode }: { countryCode: string | null }) {
  const [balance, setBalance] = useState<number | null>(null)
  const [userId, setUserId] = useState<string | null>(null)
  const [userEmail, setUserEmail] = useState<string | null>(null)
  // Paddle 이 돌려준 현지화 가격 문자열 (그대로 표시 — 프론트에서 계산/재포맷 없음)
  ...
  useEffect(() => {
    ...
    initializePaddle({
      ...
      if (!p) return
      paddleRef.current = p
      // 국가별 현지화 가격 — 서버가 헤더에서 읽은 국가를 넘기고, 없으면 Paddle 이 IP 로 자동 감지
      try {
        const items = CREDIT_PACKS.map(k => ({ priceId: packPriceId(k.key)!, quantity: 1 }))
        const res = await p.PricePreview({ items, ...(countryCode ? { address: { countryCode } } : {}) })
        const map: Record<string, string> = {}
        for (const li of res.data.details.lineItems) map[li.price.id] = li.formattedTotals.total
        setLocalized(map)
      } catch (e) { console.warn('[credits] price preview failed', e) }
    })
  ...
  const buy = (key: 'small' | 'medium' | 'large') => {
    ...
    paddleRef.current.Checkout.open({
      ...
      // 오버레이 · 한 페이지 결제창, 사이트 언어(KO/EN)와 동기화 — 그 외 언어는 Paddle이 브라우저 기준 자동 감지
            ...
            return (
              <div key={p.key} className={`relative rounded-3xl p-[1.5px] ${popular ? 'bg-gradient-to-b from-[#2563eb] to-[#06b6d4] shadow-[0_24px_60px_-24px_rgba(37,99,235,0.6)] md:-translate-y-2' : 'bg-[#ebe4d6]'}`}>
                ...
                <div className="rounded-[22.5px] bg-white p-6 md:p-7 h-full flex flex-col">
                  ...
                  <div className="mt-2 flex items-end gap-1"><span className="text-[40px] leading-none font-extrabold tracking-tight text-[#241f17]">{localized[packPriceId(p.key) ?? ''] ?? `$${p.usd}`}</span><span className="text-[13px] text-[#9d9280] mb-1.5">1회 결제</span></div>
