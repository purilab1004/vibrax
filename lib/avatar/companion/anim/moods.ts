...
// 포즈: 상반신 체중이동을 랜덤 전환. Spine 회전(Head/팔/Chest는 FK 상속 → 전신 흔들림).
// vs = [out, hold, 0] — 기울였다(out) 잠깐 유지(hold) 후 반드시 중심(0)으로 복귀(back).
//   → 한쪽으로 기운 채 멈춰 있지 않고, 움직임이 끝나면 정면/중심으로 돌아옴.
//   delay 동안엔 hold-last로 중심(0) 유지. ease 2.6 = 완만한 무게이동.
  ...
  alt: [
    {
      name: 'pose', ease: 2.6,
      delay: [2500, 6000],
      dt: [[900, 1500], [1400, 2800], [1100, 1800]],
      vs: { 'spine.x': [0.0, 0.0, 0], 'spine.y': [0.06, 0.06, 0], 'spine.z': [0.05, 0.05, 0] },
    },
    {
      name: 'pose', ease: 2.6,
      delay: [2500, 6000],
      dt: [[900, 1500], [1400, 2800], [1100, 1800]],
      vs: { 'spine.x': [0.03, 0.03, 0], 'spine.y': [-0.08, -0.08, 0], 'spine.z': [-0.06, -0.06, 0] },
    },
    {
      name: 'pose', ease: 2.6,
      delay: [2500, 6000],
      dt: [[900, 1500], [1400, 2800], [1100, 1800]],
      vs: { 'spine.x': [-0.02, -0.02, 0], 'spine.y': [0.1, 0.1, 0], 'spine.z': [0.04, 0.04, 0] },
    },
    {
      name: 'pose', ease: 2.6,
      delay: [2500, 6000],
      dt: [[800, 1300], [1200, 2400], [1000, 1600]],
      vs: { 'spine.x': [0.05, 0.05, 0], 'spine.y': [0.0, 0.0, 0], 'spine.z': [-0.03, -0.03, 0] },
    },
    {
      name: 'pose', ease: 2.6,
      delay: [2000, 5500],
      dt: [[800, 1300], [1000, 2200], [1000, 1600]],
      vs: { 'spine.x': [0.0, 0.0, 0], 'spine.y': [0.13, 0.13, 0], 'spine.z': [-0.04, -0.04, 0] },
    },
    {
      name: 'pose', ease: 2.6,
      delay: [2000, 5500],
      dt: [[800, 1300], [1000, 2200], [1000, 1600]],
      vs: { 'spine.x': [0.01, 0.01, 0], 'spine.y': [-0.12, -0.12, 0], 'spine.z': [0.06, 0.06, 0] },
    },
    // 크게 비틀어 둘러보기 (좌/우) — 돌렸다 다시 정면(0)으로 복귀
    {
      name: 'pose', p: 0.15, ease: 2.6,
      delay: [2500, 6000],
      dt: [[1300, 1900], [900, 1800], [1300, 2000]],
      vs: { 'spine.x': [0.02, 0.02, 0], 'spine.y': [0.32, 0.32, 0], 'spine.z': [-0.05, -0.05, 0] },
    },
    {
      name: 'pose', p: 0.15, ease: 2.6,
      delay: [2500, 6000],
