 ...
 * @param {{ interactive?: boolean, shadows?: boolean, zoom?: number }} [opts]  zoom>1 이면 카메라를 그만큼 뒤로 (얼굴 전체가 보이게)
 */
export function createJeumtoViewer(container, opts = {}) {
  ...
  const BASE_DIST = 5.2 * (opts.zoom || 1);
  ...
  function snapshot(size = 512, { blink = false, talk = false } = {}) {
    ...
    cam.position.set(0, 0.55, BASE_DIST * 1.22); cam.lookAt(target);
