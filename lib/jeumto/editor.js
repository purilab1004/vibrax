...
export const CLAY_COLORS = ['#e8a3a0', '#e8b89a', '#c9a27e', '#8fa4c9', '#b8b39a', '#7d8794', '#f04e98', '#f7c948', '#f2f2f2', '#2a2a2e', '#39d353', '#b39ddb'];
...
export function createJeumtoEditor(root, opts = {}) {
  ...
  function snapshot(size = 512) {
    ...
    const s = Math.min(src.width, src.height) * 0.78;
    out.width = out.height = size;
    out.getContext('2d').drawImage(src, (src.width - s) / 2, (src.height - s) / 2 - src.height * 0.03, s, s, 0, 0, size, size);
  ...
  resize();
  $('stats').textContent = `vertex ${clay.vertexCount.toLocaleString()} · faces ${clay.faceCount.toLocaleString()}`;
  setMode('sculpt');
  ...
  return {
    character, clay, state,
    snapshot, loadCharacterData, newCharacter, setMode, toast, dispose,
