...
const MAX_OFFSET = 0.8; // max distance any vertex may travel from the base shape
...
export class Clay {
  constructor(color = '#e8a3a0') {
    this.geometry = makeClayGeometry(CLAY_DEFAULTS);
    ...
    this.material = new THREE.MeshPhysicalMaterial({
      color: '#ffffff', vertexColors: true, roughness: 0.42, metalness: 0, clearcoat: 0.25, clearcoatRoughness: 0.4, sheen: 0.2,
    });
  ...
  _updateColors() {
    ...
    for (let v = 0, i = 0; v < sh.length; v++, i += 3) {
      const s = sh[v] / 255;
      col[i] = p[i] * s; col[i + 1] = p[i + 1] * s; col[i + 2] = p[i + 2] * s;
    }
  ...
  _computeCavity() {
    ...
    const DARK = 0.42, BRIGHT = 0.12; // strength of darkening / brightening
  ...
  serialize() {
    ...
    const out = { color: this.getColor(), base: { ...CLAY_DEFAULTS }, offsets16: bytesToBase64(new Uint8Array(q.buffer)) };
