...
export class Character {
  constructor(clay) {
    ...
    clay.onRebuild = () => this.clearParts(); // 토폴로지가 바뀌면 앵커가 무효
  }

  /** 로컬 좌표 point 에 가장 가까운 정점을 품은 삼각형 → 앵커 (프리셋/자동 배치용) */
  anchorNearest(point, { frontOnly = false } = {}) {
    const pos = this.clay.positions;
    let best = 0, bd = Infinity;
    for (let i = 0; i < pos.count; i++) {
      if (frontOnly && pos.getZ(i) < 0) continue;
      const dx = pos.getX(i) - point.x, dy = pos.getY(i) - point.y, dz = pos.getZ(i) - point.z;
      const d = dx * dx + dy * dy + dz * dz;
      if (d < bd) { bd = d; best = i; }
    }
    const idx = this.clay.geometry.index.array;
  ...
  /** 정면(+Z)에서 본 2D 위치(x,y: -1..1, 머리 크기 기준) → 앞면 표면 앵커. 표면을 레이캐스트해서 찾는다. */
  anchorOnFront(u, v) {
    const b = this.clay.base;
    const origin = new THREE.Vector3(u * b.width * 0.5, v * b.height * 0.5, b.depth + 3);
    const rc = new THREE.Raycaster(origin, new THREE.Vector3(0, 0, -1));
    this.clay.mesh.updateMatrixWorld(true);
    const hit = rc.intersectObject(this.clay.mesh, false)[0];
    if (hit) return this.anchorFromHit(hit);
    return this.anchorNearest(new THREE.Vector3(origin.x, origin.y, b.depth / 2), { frontOnly: true });
  }
  ...
  load(data) {
    ...
    this.presetId = data.preset ?? null;
    this.clearParts();
...
export class PartInstance {
  ...
  setHighlight(on) {
    if (!this.inner) return; // 이미 dispose 된 파츠(교체/삭제 후 남은 참조)면 무시
