...
<div id="app">
  <aside id="panel">
    ...
    <section>
      ...
      <div class="row">
        <label class="file-btn primary" title="사진·그림을 올리면 색·귀·눈·입을 알아서 얹어줘요 (시작점)">📷 사진으로 자동 빚기<input type="file" id="from-image" accept="image/png,image/jpeg,image/webp,image/gif" hidden /></label>
      </div>
      <p class="hint" id="from-image-hint">모양을 고르면 지금 점토가 새 모양으로 바뀌어요. 사진은 캐릭터/얼굴이 크게 나온 것이 좋아요.</p>
    </section>
    ...
    <section id="paint-section">
      ...
      <label>직접 선택 <input type="color" id="clay-color" value="#7fd8e6" /></label>
